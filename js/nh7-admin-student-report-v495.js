(function reportScriptV495(){
  'use strict';
  if(window.__NH7_ADMIN_STUDENT_REPORT_V495__)return;
  window.__NH7_ADMIN_STUDENT_REPORT_V495__=true;
  const VERSION='4.9.5-student-report-manual-pdf-layout';
  ...
  function panel(){
    ...
    <div class="grid3"><select id="nh7ReportStudent"><option value="">— ${E(L('انتخاب دانشجو','Choose student','Odaberi studenta'))} —</option>${opts}</select><select id="nh7ReportLang"><option value="fa">فارسی</option><option value="en">English</option><option value="hr">Hrvatski</option></select><button class="btn primary" onclick="nh7GenerateStudentReportV495()">📊 ${E(L('ساخت گزارش','Generate report','Izradi izvještaj'))}</button></div>
    <div class="actions"><button id="nh7ReportPrintBtn" class="btn secondary" onclick="nh7OpenStudentReportPreviewV495()" ${report.html?'':'disabled'}>📄 PDF / Print</button><button class="btn ghost" onclick="loadAll(true)">⟳ ${E(typeof tr==='function'?tr('refresh'):'Refresh')}</button></div>
      ...
      <div class="nh7r491-shell">
        <div class="nh7r491-toolbar">
          ...
          <div class="nh7r491-toolbar-actions">
            <button type="button" class="btn ghost" onclick="nh7CloseStudentReportPreviewV495()">✕ ${E(L('بستن','Close','Zatvori',report.language))}</button>
            <button id="nh7ReportPrintAction" type="button" class="btn secondary" onclick="nh7PrintStudentReportPdfV495()" disabled>🖨 ${E(L('چاپ','Print','Ispis',report.language))}</button>
            <button id="nh7ReportPdfAction" type="button" class="btn primary" onclick="nh7DownloadStudentReportPdfV495()" disabled>📄 ${E(L('ذخیره PDF','Save PDF','Spremi PDF',report.language))}</button>
          </div>
        </div>
  ...
  function install(){
    ...
    if(tabsHtml.__nh7Report495)return true;
    ...
    wrappedTabs.__nh7Report495=true;tabsHtml=window.tabsHtml=wrappedTabs;
    ...
    window.nh7GenerateStudentReportV495=generate;
    window.nh7OpenStudentReportPreviewV495=openPreview;
    window.nh7CloseStudentReportPreviewV495=closePreview;
    window.nh7DownloadStudentReportPdfV495=downloadPreparedPdf;
    window.nh7PrintStudentReportPdfV495=printPreparedPdf;
