/* New Hope 7 v4.2.2 — auth-safe persistent offline data.
   Prevents stale Supabase catalogue responses from hiding fresh sermons when a user token expires. */
(()=>{'use strict';
const VERSION='4.2.2-auth-safe-catalogue';
const BUILD='2.3.9.50-offline422';
const CACHE='nh7-data-stable-v329';
const SESSION='nh7_user_session_v170';
const READY_KEY='nh7_offline_data_ready';
const REMOTE_CACHE_MIGRATION='nh7_remote_catalogue_cache_v422';
...
function cacheModeOf(input,init){try{return String(init?.cache||(input instanceof Request?input.cache:'')||'default').toLowerCase()}catch(_){return'default'}}
async function cachedFetch(input,init,url,key,{remote=false}={}){
  const bypass=remote&&cacheModeOf(input,init)==='no-store';
  if(bypass)return originalFetch(input,Object.assign({},init,{cache:'no-store'}));
  const saved=await hit(key);
  if(!navigator.onLine&&saved)return saved.clone();
  try{
    const response=await originalFetch(input,remote?Object.assign({},init,{cache:'no-store'}):init);
    if(response?.ok){await store(key,response);return response}
    // Authentication and permission failures must reach cloudFetch so it can refresh
    // the user's JWT instead of silently returning an old catalogue snapshot.
    if(remote&&response&&response.status>=400&&response.status<500)return response;
    if(saved)return saved.clone();
    return response;
  }catch(error){if(saved)return saved.clone();throw error}
}
window.fetch=async function nh7OfflineDataFetch(input,init={}){const url=urlOf(input),method=methodOf(input,init);if(method==='GET'&&localData(url))return cachedFetch(input,init,url,localKey(url));if(method==='GET'&&remoteCatalogue(url))return cachedFetch(input,init,url,remoteKey(url),{remote:true});return originalFetch(input,init)};
async function clearLegacyRemoteCatalogueCache(){
  if(localStorage.getItem(REMOTE_CACHE_MIGRATION)==='1'||!('caches'in window))return;
  try{
    const cache=await caches.open(CACHE),keys=await cache.keys();
    await Promise.all(keys.filter(request=>{try{return new URL(request.url).pathname.includes('/__nh7_api_cache_v329__/')}catch(_){return false}}).map(request=>cache.delete(request)));
    localStorage.setItem(REMOTE_CACHE_MIGRATION,'1');
  }catch(error){console.warn('NH7 remote catalogue cache migration',error)}
}
clearLegacyRemoteCatalogueCache();
