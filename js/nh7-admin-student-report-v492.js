(function reportScriptV492(){
  'use strict';
  if(window.__NH7_ADMIN_STUDENT_REPORT_V492__)return;
  window.__NH7_ADMIN_STUDENT_REPORT_V492__=true;
  const VERSION='4.9.2-student-report-real-pdf';
  ...
  async function createPdfBlob(){
    ...
    holder.className='nh7r492-pdf-source';
    holder.dir=report.language==='fa'?'rtl':'ltr';
    holder.innerHTML=report.html;
    Object.assign(holder.style,{position:'fixed',left:'-12000px',top:'0',width:'1120px',padding:'22px',background:'#fff',zIndex:'-1'});
    document.body.appendChild(holder);
    try{
      const images=[...holder.querySelectorAll('img')];
      await Promise.all(images.map(img=>img.complete?Promise.resolve():new Promise(resolve=>{
        const done=()=>resolve();img.addEventListener('load',done,{once:true});img.addEventListener('error',done,{once:true});setTimeout(done,2500);
      })));
      const worker=window.html2pdf().set({
        margin:[7,7,7,7],
        filename:safeFileName(report.name||report.email),
        image:{type:'jpeg',quality:0.96},
        html2canvas:{scale:1.6,useCORS:true,backgroundColor:'#ffffff',logging:false,scrollX:0,scrollY:0,windowWidth:1180},
        jsPDF:{unit:'mm',format:'a4',orientation:'landscape',compress:true},
        pagebreak:{mode:['css','legacy'],avoid:['tr','.nh7r490-attempt']}
      }).from(holder);
      const blob=await worker.outputPdf('blob');
      if(!(blob instanceof Blob)||blob.size<500)throw new Error('Generated PDF is empty');
      return blob;
  ...
  function panel(){
    ...
    <div class="grid3"><select id="nh7ReportStudent"><option value="">— ${E(L('انتخاب دانشجو','Choose student','Odaberi studenta'))} —</option>${opts}</select><select id="nh7ReportLang"><option value="fa">فارسی</option><option value="en">English</option><option value="hr">Hrvatski</option></select><button class="btn primary" onclick="nh7GenerateStudentReportV492()">📊 ${E(L('ساخت گزارش','Generate report','Izradi izvještaj'))}</button></div>
    <div class="actions"><button id="nh7ReportPrintBtn" class="btn secondary" onclick="nh7OpenStudentReportPreviewV492()" ${report.html?'':'disabled'}>📄 PDF / Print</button><button class="btn ghost" onclick="loadAll(true)">⟳ ${E(typeof tr==='function'?tr('refresh'):'Refresh')}</button></div>
      ...
      <div class="nh7r491-shell">
        <div class="nh7r491-toolbar">
          ...
          <div class="nh7r491-toolbar-actions">
            <button type="button" class="btn ghost" onclick="nh7CloseStudentReportPreviewV492()">✕ ${E(L('بستن','Close','Zatvori',report.language))}</button>
            <button id="nh7ReportPrintAction" type="button" class="btn secondary" onclick="nh7PrintStudentReportPdfV492()" disabled>🖨 ${E(L('چاپ','Print','Ispis',report.language))}</button>
            <button id="nh7ReportPdfAction" type="button" class="btn primary" onclick="nh7DownloadStudentReportPdfV492()" disabled>📄 ${E(L('ذخیره PDF','Save PDF','Spremi PDF',report.language))}</button>
          </div>
        </div>
  ...
  function install(){
    ...
    if(tabsHtml.__nh7Report492)return true;
    ...
    wrappedTabs.__nh7Report492=true;tabsHtml=window.tabsHtml=wrappedTabs;
    ...
    window.nh7GenerateStudentReportV492=generate;
    window.nh7OpenStudentReportPreviewV492=openPreview;
    window.nh7CloseStudentReportPreviewV492=closePreview;
    window.nh7DownloadStudentReportPdfV492=downloadPreparedPdf;
    window.nh7PrintStudentReportPdfV492=printPreparedPdf;
