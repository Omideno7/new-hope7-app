(function reportScriptV491(){
  'use strict';
  if(window.__NH7_ADMIN_STUDENT_REPORT_V491__)return;
  window.__NH7_ADMIN_STUDENT_REPORT_V491__=true;
  const VERSION='4.9.1-student-report-mobile-preview';
  ...
  function panel(){
    ...
    <div class="grid3"><select id="nh7ReportStudent"><option value="">— ${E(L('انتخاب دانشجو','Choose student','Odaberi studenta'))} —</option>${opts}</select><select id="nh7ReportLang"><option value="fa">فارسی</option><option value="en">English</option><option value="hr">Hrvatski</option></select><button class="btn primary" onclick="nh7GenerateStudentReportV491()">📊 ${E(L('ساخت گزارش','Generate report','Izradi izvještaj'))}</button></div>
    <div class="actions"><button id="nh7ReportPrintBtn" class="btn secondary" onclick="nh7OpenStudentReportPreviewV491()" ${report.html?'':'disabled'}>📄 PDF / Print</button><button class="btn ghost" onclick="loadAll(true)">⟳ ${E(typeof tr==='function'?tr('refresh'):'Refresh')}</button></div>
  ...
  function printCurrent(mode='print'){
    if(!report.html)return;
    const cleanup=()=>document.body.classList.remove('nh7r491-printing');
    document.body.classList.add('nh7r491-printing');
    const note=document.getElementById('nh7ReportPreviewHint');
    if(note)note.textContent=mode==='pdf'
      ?L('پنجره چاپ باز می‌شود؛ در iPhone از Share سپس Save to Files برای ذخیره PDF استفاده کن.','The print sheet will open; on iPhone use Share then Save to Files to save the PDF.','Otvorit će se ispis; na iPhoneu odaberite Share pa Save to Files za PDF.')
      :L('پنجره چاپ دستگاه باز می‌شود.','The device print dialog will open.','Otvorit će se dijalog za ispis.');
    const after=()=>{cleanup();window.removeEventListener('afterprint',after)};
    window.addEventListener('afterprint',after);
    setTimeout(()=>{try{window.print()}finally{setTimeout(cleanup,1200)}},80);
  }
      ...
      <div class="nh7r491-shell">
        <div class="nh7r491-toolbar">
          ...
          <div class="nh7r491-toolbar-actions">
            <button type="button" class="btn ghost" onclick="nh7CloseStudentReportPreviewV491()">✕ ${E(L('بستن','Close','Zatvori',report.language))}</button>
            <button type="button" class="btn secondary" onclick="nh7PrintStudentReportNowV491('print')">🖨 ${E(L('چاپ','Print','Ispis',report.language))}</button>
            <button type="button" class="btn primary" onclick="nh7PrintStudentReportNowV491('pdf')">📄 ${E(L('ذخیره PDF','Save PDF','Spremi PDF',report.language))}</button>
          </div>
        </div>
  ...
  function install(){
    ...
    if(tabsHtml.__nh7Report491)return true;
    ...
    wrappedTabs.__nh7Report491=true;tabsHtml=window.tabsHtml=wrappedTabs;
    ...
    window.nh7GenerateStudentReportV491=generate;
    window.nh7OpenStudentReportPreviewV491=openPreview;
    window.nh7CloseStudentReportPreviewV491=closePreview;
    window.nh7PrintStudentReportNowV491=printCurrent;
