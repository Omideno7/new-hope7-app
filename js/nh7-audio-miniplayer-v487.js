...
(()=>{'use strict';
if(window.__NH7_AUDIO_MINIPLAYER_V487__)return;
window.__NH7_AUDIO_MINIPLAYER_V487__=true;

const VERSION='4.8.7-collapsible-miniplayer-resume-close';
...
function writeResumeSnapshot(snapshot){
  if(!snapshot?.id)return;
  try{
    localStorage.setItem('nh7_sermon_progress_'+snapshot.id,JSON.stringify({
      time:Number(snapshot.time||0),
      duration:Number(snapshot.duration||0),
      completed:false,
      updatedAt:new Date().toISOString()
    }));
    localStorage.setItem('nh7_audio_close_resume_v487:'+snapshot.id,JSON.stringify(snapshot));
  }catch(_){}
}
function persistProgressBeforeClose(){
  const s=state(),a=s.audio,item=s.current;if(!a||!item)return null;
  const id=String(item?.id||item?.analytics_id||'');if(!id)return null;
  const snapshot={
    id,
    time:Math.max(0,Number(a.currentTime||0)),
...
function closeMiniPlayer(){
  ...
  const resume=persistProgressBeforeClose();
  setExpanded(false);
  if(a){
    try{a.pause()}catch(_){}
    setTimeout(()=>{
      try{a.removeAttribute('src');a.load()}catch(_){}
      // load()/emptied can trigger a final timeupdate at 0. Re-assert the
      // captured resume point after unload so closing never erases progress.
      if(resume){
        writeResumeSnapshot(resume);
        setTimeout(()=>writeResumeSnapshot(resume),140);
        setTimeout(()=>writeResumeSnapshot(resume),500);
      }
    ...
    if(resume)writeResumeSnapshot(resume);
    clearSystemMediaSession();
    if(root)root.hidden=true;
  }
}
...
window.NH7_AUDIO_MINIPLAYER_V487={sync,setExpanded,close:closeMiniPlayer,writeResumeSnapshot,get expanded(){return expanded}};
})();