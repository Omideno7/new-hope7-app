/* New Hope 7 v3.3.0 — one-time background preparation of the offline app shell */
(()=>{'use strict';
const VERSION='3.3.0-offline-core';
...
async function prepare(force=false){
  if(running||!navigator.onLine||(!force&&ready()))return null;
  running=true;
  try{const result=await send('CACHE_CORE');localStorage.setItem(READY_KEY,JSON.stringify({build:BUILD,cached:Number(result?.cached||0),at:new Date().toISOString()}));try{await navigator.storage?.persist?.()}catch(_){}return result}
