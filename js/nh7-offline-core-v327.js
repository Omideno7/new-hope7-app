/* New Hope 7 v3.2.9 — background preparation of the complete offline app shell */
(()=>{'use strict';
const VERSION='3.2.9-offline-core';
const BUILD='2.3.9.29';
let running=false;
function send(type,payload={}){return new Promise(async(resolve,reject)=>{if(!('serviceWorker'in navigator)){reject(new Error('Service worker unavailable'));return}let timer=0;try{const registration=await Promise.race([navigator.serviceWorker.ready,new Promise((_,r)=>setTimeout(()=>r(new Error('Service worker readiness timed out')),7000))]);const target=navigator.serviceWorker.controller||registration.active||registration.waiting;if(!target)throw new Error('Service worker inactive');const channel=new MessageChannel();timer=setTimeout(()=>reject(new Error('Offline core preparation timed out')),180000);channel.port1.onmessage=event=>{clearTimeout(timer);const data=event.data||{};data.ok===false?reject(new Error(data.error||'Offline preparation failed')):resolve(data)};target.postMessage(Object.assign({type},payload),[channel.port2])}catch(error){clearTimeout(timer);reject(error)}})}
