/* New Hope 7 — classic signed audio player v4.8.2
 *
...
let audio=null,current=null,currentPanel=null,patchTimer=0,prewarmBusy=false,nowPlayingBar=null;
...
function syncNowPlaying(){
  ...
  const title=bar.querySelector('[data-now-title]'),time=bar.querySelector('[data-now-time]'),toggle=bar.querySelector('[data-now-toggle]');
...
function ensureAudio(){
  ...
  audio.addEventListener('ended',()=>{if(!current)return;captureListen();saveProgress(current,true);setStatus(current,L('پخش کامل شد ✓','Completed ✓','Završeno ✓'),'ok');syncPanel(current);syncNowPlaying();flushTracking(true,true)});
...
window.NH7_AUDIO_CLASSIC_VERSION='4.8.2';
window.NH7_AUDIO_SIGNED_VERSION='4.6.1-401-refresh';
window.NH7_AUDIO_CLASSIC_V400={patch,prewarm,playItem,downloadItem,clearAll,getState:()=>({current,audio}),openCurrentAudio,syncNowPlaying};
