...
function selectionHtml(){
  const list=rows().slice().sort((a,b)=>String(b.updated_at||b.approved_at||b.created_at||'').localeCompare(String(a.updated_at||a.approved_at||a.created_at||''))),selected=currentId();
  if(!list.length)return `<section class="nh7-f5-cert-select"><div><strong>📄 ${E(L('انتخاب مدرک برای ویرایش','Choose a document to edit','Odaberite dokument za uređivanje'))}</strong><p>${E(L('هنوز مدرک صادرشده‌ای در فهرست نیست. پنجره را ببندید، مدرک جدید را صادر کنید و سپس دوباره استودیو را باز کنید.','There are no issued documents yet. Close the studio, issue a document, then reopen it.','Još nema izdanih dokumenata.'))}</p></div><button type="button" data-f5-new-document>${E(L('رفتن به فرم صدور مدرک','Go to issue form','Idi na obrazac izdavanja'))}</button></section>`;
  return `<section class="nh7-f5-cert-select"><div class="nh7-f5-cert-copy"><strong>📄 ${E(L('مدرک موردنظر برای ویرایش','Document to edit','Dokument za uređivanje'))}</strong><p>${E(L('یک مدرک صادرشده را انتخاب و «بارگذاری برای ویرایش» را بزنید. طراحی ذخیره‌شده همان مدرک داخل استودیو باز می‌شود.','Select an issued document and load it for editing.','Odaberite izdani dokument i učitajte ga za uređivanje.'))}</p></div><div class="nh7-f5-cert-actions"><select data-f5-certificate><option value="">${E(L('انتخاب مدرک…','Choose document…','Odaberite dokument…'))}</option>${list.map(row=>`<option value="${E(row.id)}" ${String(row.id)===selected?'selected':''}>${E(rowLabel(row))}</option>`).join('')}</select><button type="button" data-f5-load>${E(L('بارگذاری برای ویرایش','Load for editing','Učitaj za uređivanje'))}</button><button type="button" data-f5-refresh>${E(L('تازه‌سازی فهرست','Refresh list','Osvježi popis'))}</button><button type="button" data-f5-new-document>${E(L('صدور مدرک جدید','Issue new document','Izdaj novi dokument'))}</button></div>${selected?`<small class="nh7-f5-selected">✓ ${E(L('مدرک انتخاب‌شده فعال است. اکنون می‌توانید طراحی را ذخیره کنید.','A document is selected. You can now save the design.','Dokument je odabran.'))}</small>`:''}</section>`;
}
function inject(){
  if(injecting)return;const controls=document.querySelector('.nh7-f4-overlay .nh7-f4-controls');if(!controls)return;injecting=true;
  try{controls.querySelector('.nh7-f5-cert-select')?.remove();controls.insertAdjacentHTML('afterbegin',selectionHtml())}finally{injecting=false}
}
