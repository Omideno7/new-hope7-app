(function reportScriptV494(){
  'use strict';
  if(window.__NH7_ADMIN_STUDENT_REPORT_V494__)return;
  window.__NH7_ADMIN_STUDENT_REPORT_V494__=true;
  const VERSION='4.9.4-student-report-a4-fit';
  ...
  async function createPdfBlob(){
    ...
    source.querySelectorAll('section').forEach(el=>{
      el.style.overflow='visible';
      el.style.width='100%';
      el.style.maxWidth='100%';
    });
    source.querySelectorAll('table').forEach(el=>{
      el.style.width='100%';
      el.style.maxWidth='100%';
      el.style.minWidth='0';
    ...
    source.querySelectorAll('td,th,.long,p,li,small,strong,span').forEach(el=>{
      el.style.overflowWrap='anywhere';
      el.style.wordBreak='break-word';
      el.style.whiteSpace=el.classList?.contains('long')?'pre-wrap':'normal';
      el.style.maxWidth='100%';
    });
    source.querySelectorAll('img').forEach(el=>{
    ...
    if(header){
      ...
      header.style.gridTemplateColumns='minmax(0,1fr) 72px';
    ...
    try{
      ...
      const overflowNodes=[...source.querySelectorAll('*')].filter(el=>{
        try{return el.scrollWidth>el.clientWidth+3}catch(_){return false}
      });
      ...
      if(!Number.isFinite(aspect)||aspect<=0)throw new Error(L('اندازه PDF معتبر نیست.','Invalid PDF dimensions.','Neispravne dimenzije PDF-a.'));
  ...
  function panel(){
    ...
    <div class="grid3"><select id="nh7ReportStudent"><option value="">— ${E(L('انتخاب دانشجو','Choose student','Odaberi studenta'))} —</option>${opts}</select><select id="nh7ReportLang"><option value="fa">فارسی</option><option value="en">English</option><option value="hr">Hrvatski</option></select><button class="btn primary" onclick="nh7GenerateStudentReportV494()">📊 ${E(L('ساخت گزارش','Generate report','Izradi izvještaj'))}</button></div>
    <div class="actions"><button id="nh7ReportPrintBtn" class="btn secondary" onclick="nh7OpenStudentReportPreviewV494()" ${report.html?'':'disabled'}>📄 PDF / Print</button><button class="btn ghost" onclick="loadAll(true)">⟳ ${E(typeof tr==='function'?tr('refresh'):'Refresh')}</button></div>
      ...
      <div class="nh7r491-shell">
        <div class="nh7r491-toolbar">
          ...
          <div class="nh7r491-toolbar-actions">
            <button type="button" class="btn ghost" onclick="nh7CloseStudentReportPreviewV494()">✕ ${E(L('بستن','Close','Zatvori',report.language))}</button>
            <button id="nh7ReportPrintAction" type="button" class="btn secondary" onclick="nh7PrintStudentReportPdfV494()" disabled>🖨 ${E(L('چاپ','Print','Ispis',report.language))}</button>
            <button id="nh7ReportPdfAction" type="button" class="btn primary" onclick="nh7DownloadStudentReportPdfV494()" disabled>📄 ${E(L('ذخیره PDF','Save PDF','Spremi PDF',report.language))}</button>
          </div>
        </div>
  ...
  function install(){
    ...
    if(tabsHtml.__nh7Report494)return true;
    ...
    wrappedTabs.__nh7Report494=true;tabsHtml=window.tabsHtml=wrappedTabs;
    ...
    window.nh7GenerateStudentReportV494=generate;
    window.nh7OpenStudentReportPreviewV494=openPreview;
    window.nh7CloseStudentReportPreviewV494=closePreview;
    window.nh7DownloadStudentReportPdfV494=downloadPreparedPdf;
    window.nh7PrintStudentReportPdfV494=printPreparedPdf;
