/* New Hope 7 v3.3.4 — safe updates that never interrupt active School Audio */
(()=>{'use strict';
...
async function moveToLatest(remote){const latest=String(remote?.app||'').trim();if(!latest||latest===BUILD)return false;if(schoolAudioBusy()){sessionStorage.setItem(PENDING_UPDATE,latest);return false}return goLatest(latest)}
...
if(navigator.serviceWorker){navigator.serviceWorker.addEventListener('controllerchange',()=>{const guard='nh7_controller_reloaded_'+BUILD;if(sessionStorage.getItem(guard)==='1')return;sessionStorage.setItem(guard,'1');safeReload()})}
window.addEventListener('nh7-school-audio-idle',()=>{const latest=sessionStorage.getItem(PENDING_UPDATE);if(latest&&latest!==BUILD){goLatest(latest);return}if(sessionStorage.getItem(PENDING_RELOAD)==='1')safeReload()});
window.addEventListener('pageshow',()=>setTimeout(check,900));document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(check,650)});window.addEventListener('online',()=>setTimeout(()=>{boot();check()},500));boot();window.NH7_AUTO_UPDATE_VERSION='3.3.4';
})();
