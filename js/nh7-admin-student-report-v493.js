(function reportScriptV493(){
  'use strict';
  if(window.__NH7_ADMIN_STUDENT_REPORT_V493__)return;
  window.__NH7_ADMIN_STUDENT_REPORT_V493__=true;
  const VERSION='4.9.3-student-report-visible-pdf-capture';
  ...
  function loadPdfEngine(){
    ...
    pdfEnginePromise=new Promise((resolve,reject)=>{
      const existing=document.querySelector('script[data-nh7-pdf-engine="1"]');
      if(existing){
      ...
      s.dataset.nh7PdfEngine='1';
      s.src='https://cdn.jsdelivr.net/npm/html2pdf.js@0.10.1/dist/html2pdf.bundle.min.js';
      s.async=true;
      const timer=setTimeout(()=>reject(new Error('PDF engine timed out')),20000);
      s.onload=()=>{clearTimeout(timer);window.html2pdf?resolve(window.html2pdf):reject(new Error('PDF engine unavailable'))};
      s.onerror=()=>{clearTimeout(timer);reject(new Error('PDF engine failed to load'))};
  ...
  async function createPdfBlob(){
    ...
    const source=document.querySelector('#nh7ReportPreviewOverlay .nh7r491-print-surface .nh7r490-report')
      ||document.querySelector('#nh7ReportOutput .nh7r490-report');
    if(!source)throw new Error(L('گزارش قابل مشاهده پیدا نشد. Preview را دوباره باز کن.','Visible report not found. Reopen the preview.','Vidljivi izvještaj nije pronađen. Ponovno otvorite pregled.'));
    const scrollHost=source.closest('.nh7r491-preview-scroll');
    const previousScroll=scrollHost?scrollHost.scrollTop:0;
    const touched=[source,...source.querySelectorAll('section,table,.long')];
    const oldStyles=touched.map(el=>el.getAttribute('style'));
    const apple=/iPad|iPhone|iPod/i.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1);
    if(scrollHost)scrollHost.scrollTop=0;
    source.style.width='1040px';
    source.style.maxWidth='1040px';
    source.style.minWidth='1040px';
    source.style.margin='0';
    source.style.padding='24px';
    source.style.background='#ffffff';
    source.style.overflow='visible';
    source.querySelectorAll('section').forEach(el=>{el.style.overflow='visible';el.style.maxWidth='none'});
    source.querySelectorAll('table').forEach(el=>{el.style.width='100%';el.style.minWidth='0';el.style.tableLayout='fixed'});
    source.querySelectorAll('.long,td,th,p,li').forEach(el=>{el.style.overflowWrap='anywhere';el.style.wordBreak='break-word'});
    ...
    try{
      ...
      const worker=window.html2pdf().set({
        margin:[7,7,7,7],
        filename:safeFileName(report.name||report.email),
        image:{type:'jpeg',quality:0.96},
        html2canvas:{
          scale:apple?1.2:1.45,
          useCORS:true,
          allowTaint:false,
        ...
        pagebreak:{mode:['css','legacy'],avoid:['tr','.nh7r490-attempt']}
      ...
      touched.forEach((el,i)=>{const old=oldStyles[i];if(old==null)el.removeAttribute('style');else el.setAttribute('style',old)});
  ...
  function panel(){
    ...
    <div class="grid3"><select id="nh7ReportStudent"><option value="">— ${E(L('انتخاب دانشجو','Choose student','Odaberi studenta'))} —</option>${opts}</select><select id="nh7ReportLang"><option value="fa">فارسی</option><option value="en">English</option><option value="hr">Hrvatski</option></select><button class="btn primary" onclick="nh7GenerateStudentReportV493()">📊 ${E(L('ساخت گزارش','Generate report','Izradi izvještaj'))}</button></div>
    <div class="actions"><button id="nh7ReportPrintBtn" class="btn secondary" onclick="nh7OpenStudentReportPreviewV493()" ${report.html?'':'disabled'}>📄 PDF / Print</button><button class="btn ghost" onclick="loadAll(true)">⟳ ${E(typeof tr==='function'?tr('refresh'):'Refresh')}</button></div>
      ...
      <div class="nh7r491-shell">
        <div class="nh7r491-toolbar">
          ...
          <div class="nh7r491-toolbar-actions">
            <button type="button" class="btn ghost" onclick="nh7CloseStudentReportPreviewV493()">✕ ${E(L('بستن','Close','Zatvori',report.language))}</button>
            <button id="nh7ReportPrintAction" type="button" class="btn secondary" onclick="nh7PrintStudentReportPdfV493()" disabled>🖨 ${E(L('چاپ','Print','Ispis',report.language))}</button>
            <button id="nh7ReportPdfAction" type="button" class="btn primary" onclick="nh7DownloadStudentReportPdfV493()" disabled>📄 ${E(L('ذخیره PDF','Save PDF','Spremi PDF',report.language))}</button>
          </div>
        </div>
  ...
  function install(){
    ...
    if(tabsHtml.__nh7Report493)return true;
    ...
    wrappedTabs.__nh7Report493=true;tabsHtml=window.tabsHtml=wrappedTabs;
    ...
    window.nh7GenerateStudentReportV493=generate;
    window.nh7OpenStudentReportPreviewV493=openPreview;
    window.nh7CloseStudentReportPreviewV493=closePreview;
    window.nh7DownloadStudentReportPdfV493=downloadPreparedPdf;
    window.nh7PrintStudentReportPdfV493=printPreparedPdf;
