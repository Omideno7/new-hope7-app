/* New Hope 7 — School Guide UI v1.0.2
 ...
 */
(()=>{'use strict';
if(window.__NH7_SCHOOL_GUIDE_V102__)return;window.__NH7_SCHOOL_GUIDE_V102__=true;
...
function markPreviewOnly(root=document){
  root.querySelectorAll?.('.nh7-school-rule-note,.nh7-school-qa-tools,.nh7-preview-reset,[data-preview-pass],[data-preview-fail]').forEach(el=>{if(el.dataset.previewOnly!=='1')el.dataset.previewOnly='1'});
}
function wrap(section){
  if(!section||section.dataset.guideWrapped==='1')return false;
  ...
  details.append(summary,body);section.appendChild(details);markPreviewOnly(section);return true;
}
function updateLabels(){
  const nextLabel=label(),nextHint=hint();
  document.querySelectorAll('[data-school-guide-label]').forEach(n=>{if(n.textContent!==nextLabel)n.textContent=nextLabel});
  document.querySelectorAll('[data-school-guide-hint]').forEach(n=>{if(n.textContent!==nextHint)n.textContent=nextHint});
}
...
const view=document.getElementById('view');if(view)new MutationObserver(scheduleScan).observe(view,{childList:true,subtree:true});
document.getElementById('langSelect')?.addEventListener('change',()=>setTimeout(scheduleScan,120));
document.addEventListener('DOMContentLoaded',scheduleScan,{once:true});setTimeout(scheduleScan,150);
window.NH7_SCHOOL_GUIDE_VERSION='1.0.2';
})();
