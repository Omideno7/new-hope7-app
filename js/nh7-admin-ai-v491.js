...
function authReady(){
  try{
    const access=(typeof nh7AdminAccessReady!=='undefined')?nh7AdminAccessReady:window.nh7AdminAccessReady;
    return Boolean(access&&typeof token==='string'&&token);
...
function qnaControls(q){
  ...
  return `<div class="nh7-ai490-qna" data-nh7-ai-qna="${E(id)}">
    <div class="nh7-ai490-actions">
      <button type="button" class="btn secondary" data-nh7-ai-button onclick="nh7AiTranslateQuestionV491('${E(id)}')">✨ ${E(L('ترجمه سؤال','Translate question','Prevedi pitanje'))}</button>
      <button type="button" class="btn primary" data-nh7-ai-button onclick="nh7AiDraftAnswerV491('${E(id)}')">🤖 ${E(L('پیش‌نویس پاسخ با AI','AI draft answer','AI nacrt odgovora'))}</button>
      <button type="button" class="btn secondary" data-nh7-ai-button onclick="nh7AiTranslateAnswerV491('${E(id)}')">✨ ${E(L('ترجمه پاسخ','Translate answer','Prevedi odgovor'))}</button>
    </div>
...
function installAssignmentPatch(){
  if(typeof renderSchoolAssignmentCard!=='function'||renderSchoolAssignmentCard.__nh7Ai491)return;
  const base=renderSchoolAssignmentCard;
  const wrapped=function(a){
    let html=base(a),marker='<div class="actions">';
    const button=`<div class="nh7-ai490-assignment"><button type="button" class="btn secondary" data-nh7-ai-button onclick="nh7AiDraftAssignmentFeedbackV491('${E(a.id)}')">🤖 ${E(L('پیش‌نویس بازخورد با AI','AI draft feedback','AI nacrt povratne informacije'))}</button><small data-nh7-ai-provider-note class="nh7-ai490-provider"></small></div>`;
    if(html.includes(marker))html=html.replace(marker,button+marker);return html;
  };
  wrapped.__nh7Ai491=true;renderSchoolAssignmentCard=window.renderSchoolAssignmentCard=wrapped;
}
