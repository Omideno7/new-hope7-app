/* New Hope 7 Admin production extension loader v2.4.0.247.
   Loads the unchanged production analytics bundle, isolated school workflow, then the mobile search stability patch. */
(()=>{'use strict';
...
load(CORE).then(()=>load(PATCH)).then(()=>load(SEARCH)).catch(err=>{console.error('[NH7 admin production loader]',err);const el=document.createElement('div');el.className='notice';el.textContent='Admin extension failed to load: '+err.message;document.getElementById('adminApp')?.prepend(el)});
})();