/* New Hope 7 Admin RC extension loader v2.5.1.
   Loads the unchanged local analytics core first, then isolated school and Account-ID access extensions. */
(()=>{'use strict';
...
load(CORE).then(()=>load(SCHOOL)).then(()=>load(ACCESS)).catch(err=>{console.error('[NH7 admin RC loader]',err);const el=document.createElement('div');el.className='notice';el.textContent='Admin extension failed to load: '+err.message;document.getElementById('adminApp')?.prepend(el)});
})();
