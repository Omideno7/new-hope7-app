...
function json(origin: string | null, body: unknown, status = 200) {
  return new Response(JSON.stringify(body), { status, headers: { ...cors(origin), 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store, private' } });
}
function clean(value: unknown) { return String(value || '').trim(); }
function tokenFrom(req: Request) { const value = req.headers.get('authorization') || ''; return value.replace(/^Bearer\s+/i, '').trim(); }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: authData, error: authError } = await admin.rpc('nh7_library_authorize_v230', {
      p_item_id: itemId,
      p_code: code,
    ...
    if (!access?.allowed) {
      ...
      return json(origin, { error: reason, code: reason }, reason === 'not_found' ? 404 : 403);
    }

    const path = clean(access.storage_path).replace(/^\/+/, '');
    if (!path || path.includes('..')) return json(origin, { error: 'Invalid storage path', code: 'invalid_path' }, 400);
    const { data: signed, error: signError } = await admin.storage.from('nh7-library').createSignedUrl(path, 600, {
    ...
    return json(origin, {
      ...
      grant_active: Boolean(access.grant_active),
    ...
    console.error('nh7-library-access', error);
