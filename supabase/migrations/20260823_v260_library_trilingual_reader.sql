-- New Hope 7 v2.6.0 - trilingual structured public-library reader.
-- Additive only: existing v2.5.0 reader RPC remains untouched.
...
begin
  -- Public-library items still use the verified school account gate already
  -- used by the production reader. Minister/account grants are handled by
  -- the existing authorization layer and are not weakened here.
  ...
  if not coalesce((v_auth->>'allowed')::boolean,false) then
    return v_auth;
  end if;
  ...
  -- Report languages that actually contain a readable payload.
  ...
  -- Requested app language first, then deterministic fallbacks.
