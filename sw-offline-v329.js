...
function recoveryRequest(url){const rel=scopeRelative(url);return rel==='reset-password.html'||rel==='admin-reset-password.html'}
function adminRequest(url){const rel=scopeRelative(url);return rel==='admin.html'||rel==='admin-v239.html'||rel==='admin-v239-stable.html'||rel==='admin-reset-password.html'||rel==='admin-refresh.html'||rel==='admin-manifest.json'||rel==='certificate-v239.html'||rel.startsWith('js/admin-')||rel.startsWith('js/nh7-admin-')||rel.startsWith('css/admin-')}
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'&&url.origin===self.location.origin){
    if(recoveryRequest(url)){event.respondWith((async()=>await updateCache(request,CORE_CACHE)||await caches.match(request,{ignoreSearch:true})||await caches.match(OFFLINE_URL,{ignoreSearch:true}))());return}
  ...
  if(url.origin===self.location.origin&&recoveryRequest(url)){event.respondWith((async()=>await updateCache(request,CORE_CACHE)||await caches.match(request,{ignoreSearch:true})||unavailable(url))());return}
