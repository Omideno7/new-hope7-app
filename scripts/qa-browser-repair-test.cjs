...
(async()=>{
 for(const [engine,type] of [['chromium',chromium],['webkit',webkit]]){
  ...
  for(const staleSession of [false,true]){
   ...
   const p=await ctx.newPage();p.on('pageerror',e=>item.errors.push({message:e.message,stack:e.stack}));
   ...
   for(const [r,expected] of [['home','Church'],['daily','Daily'],['bible','Bible'],['audio','School'],['more','Settings']]){
 ...
 console.log('QA_BROWSER_REPORT',JSON.stringify(report,null,2));
 clearTimeout(safetyDeadline);
})().catch(e=>{report.passed=false;report.failure=String(e);fs.writeFileSync(path.join(OUT,'browser-report.json'),JSON.stringify(report,null,2));console.error('QA_BROWSER_REPORT',JSON.stringify(report,null,2));process.exit(1)});
