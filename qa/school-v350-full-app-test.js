...
(async () => {
  ...
  let mediaAccessCalls = 0, mediaLessonCode = '';
  ...
  await page.route('**/*', async route => {
    ...
    if (u.hostname === 'gpzcwffxnddhaeaogdyo.supabase.co') {
      if (u.pathname.endsWith('/functions/v1/nh7-school-media-access')) {
        mediaAccessCalls++;
        try { mediaLessonCode = String(route.request().postDataJSON()?.lesson_code || ''); } catch (_) {}
        return route.fulfill({status:200,contentType:'application/json',headers:cors,body:JSON.stringify({signed_url:'https://media.example.test/signed-school-class-1.mp3',expires_in:900,mime_type:'audio/mpeg'})});
      }
  ...
  if (mapped !== 'https://media.example.test/school-class-1.mp3') throw Error('raw School audio metadata changed');

  // The authoritative player deliberately uses a signed URL for School audio.
  await page.locator('[data-sermon-play="school-class_01_new_creation"]').click({force:true});
  await page.waitForFunction(()=>document.querySelector('body > audio')?.src.includes('signed-school-class-1.mp3'),null,{timeout:5000});
  const audioSrc = await page.evaluate(()=>document.querySelector('body > audio')?.src||'');
  if (!audioSrc.includes('signed-school-class-1.mp3')) throw Error('authoritative player did not receive signed School audio');
  if (mediaAccessCalls < 1 || mediaLessonCode !== 'class_01_new_creation') throw Error(`secure media handoff wrong: calls=${mediaAccessCalls} lesson=${mediaLessonCode}`);

  console.log(JSON.stringify({FULL_APP_SCHOOL_V350_OK:true,schoolMs:Date.now()-started,stages:7,originalLessons:8,class4Lessons:2,textPreserved:true,audioMetadataPreserved:true,signedAudioHandoff:true,assignmentPreserved:true,mediaAccessCalls}));
