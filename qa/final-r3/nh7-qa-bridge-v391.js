/* New Hope 7 Final QA R3 v3.9.1 — bridge approved RC UI to current secure backends. */
(()=>{'use strict';
if(window.__NH7_QA_BRIDGE_V391__)return;window.__NH7_QA_BRIDGE_V391__=true;
...
function blockLegacyArchive(){
  window.__NH7_SERMON_ARCHIVE_FALLBACK_BLOCKED__={at:Date.now(),reason:'live_archive_required'};
  return new Response(JSON.stringify({version:'3.9.1-live-archive-only',categories:[]}),{status:200,headers:{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'}});
}
async function markLiveSermonResponse(response){
  if(!response?.ok)return;
  try{
    const rows=await response.clone().json();
    if(Array.isArray(rows)&&rows.length){
      window.__NH7_SERMON_ARCHIVE_LIVE__={at:Date.now(),count:rows.length};
      window.__NH7_SERMON_ARCHIVE_FALLBACK_BLOCKED__=null;
      window.dispatchEvent(new CustomEvent('nh7:sermon-archive-live',{detail:{count:rows.length}}));
    }
  }catch(_){ }
}
window.fetch=async function nh7QaBridgeFetch(input,init={}){
  ...
  // Never allow this QA build to silently show the obsolete first-version bundled archive.
  // The current cloud archive is the only valid sermon source.
  if(/\/data\/audio\/messages\.json$/i.test(path))return blockLegacyArchive();
  ...
  if(path.endsWith('/rest/v1/sermons')){
    window.__NH7_SERMON_ARCHIVE_REQUESTED__={at:Date.now()};
    const response=await nativeFetch(input,Object.assign({},init,{cache:'no-store'}));
    markLiveSermonResponse(response);
    return response;
  }
  if(path.endsWith('/rest/v1/sermon_categories'))return nativeFetch(input,Object.assign({},init,{cache:'no-store'}));
...
window.NH7_QA_BRIDGE_VERSION='3.9.1';
})();
