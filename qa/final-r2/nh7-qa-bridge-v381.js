...
const APO='https://raw.githack.com/Omideno7/new-hope7-app/dff84647ae2aaff7c02cd81f28d9997d7c15a135/data/apocrypha/runtime/apocrypha-browser-19.preview.json?v=3810';
...
async function mirrorSchoolAudio(input,init,body){
  ...
  const headers=authHeaders(input,init);nativeFetch(`${SB}/rest/v1/rpc/nh7_school_record_audio_v380`,{method:'POST',headers,body:JSON.stringify(payload),cache:'no-store'}).catch(e=>console.warn('[NH7 QA audio mirror]',e));
}
window.fetch=async function nh7QaBridgeFetch(input,init={}){
  ...
  if(path.endsWith('/data/apocrypha/runtime/apocrypha-browser-19.preview.json'))return nativeFetch(APO,{cache:'no-store'});
  ...
  if(target){const next=cloneInit(input,init),b=parseBody(init);next.body=JSON.stringify(bodyMapper?bodyMapper(b):b);next.cache='no-store';return nativeFetch(replaceRpc(url.href,target),next)}
  if(/\/rest\/v1\/rpc\/nh7_track_audio_session_v22[12]$/.test(path)){const b=parseBody(init),res=await nativeFetch(input,init);mirrorSchoolAudio(input,init,b);return res}
  if(url.origin===SB&&path.endsWith('/auth/v1/recover')&&String(init?.method||'POST').toUpperCase()==='POST'){
    const b=parseBody(init),redirect=url.searchParams.get('redirect_to')||'';if(!/admin-reset-password\.html/i.test(redirect)){const email=String(b?.email||'').trim().toLowerCase(),ok=await recoveryAllowed(email).catch(()=>false);if(!ok){const message=lang()==='fa'?'بازیابی رمز فقط برای حسابی فعال است که ثبت‌نام آن کامل و توسط ادمین تأیید شده باشد.':lang()==='hr'?'Obnova lozinke dostupna je samo za potpuno registriran i odobren račun.':'Password recovery is available only for a fully registered and admin-approved account.';return new Response(JSON.stringify({code:'recovery_not_eligible',message}),{status:403,headers:{'Content-Type':'application/json'}})}}
  }
