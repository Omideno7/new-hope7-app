...
const VERSION='v2.3.9.23-offline-persistence';
...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{await migrateLegacyMedia();const keep=new Set([CORE_CACHE,DATA_CACHE,PUBLIC_API_CACHE,MEDIA_CACHE]);const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('nh7-')||k.startsWith('omideno7-')).filter(k=>!keep.has(k)).map(k=>caches.delete(k)));await self.clients.claim()})())});
...
async function stableCachedResponse(raw){const cache=await caches.open(MEDIA_CACHE),stable=stableMediaKey(raw);let response=await cache.match(stable);if(response)return response;const direct=await cache.match(new Request(raw,{method:'GET'}));if(direct){await cache.put(stable,direct.clone());await cache.delete(new Request(raw,{method:'GET'}));return direct}const identity=canonicalMediaIdentity(raw),keys=await cache.keys();for(const candidate of keys){if(new URL(candidate.url).pathname.includes('/'+STABLE_MEDIA_PATH+'/'))continue;if(canonicalMediaIdentity(candidate.url)!==identity)continue;response=await cache.match(candidate);if(response){await cache.put(stable,response.clone());await cache.delete(candidate);return response}}return null}
async function mediaFromCache(request){const full=await stableCachedResponse(request.url);if(!full)return null;const range=request.headers.get('range');if(!range)return full;const blob=await full.blob();const m=/bytes=(\d+)-(\d*)/.exec(range);if(!m)return full;const start=Number(m[1]),end=m[2]?Number(m[2]):blob.size-1;if(start>=blob.size)return new Response(null,{status:416,headers:{'Content-Range':`bytes */${blob.size}`}});const finalEnd=Math.min(end,blob.size-1),chunk=blob.slice(start,finalEnd+1),headers=new Headers(full.headers);headers.set('Content-Range',`bytes ${start}-${finalEnd}/${blob.size}`);headers.set('Content-Length',String(chunk.size));headers.set('Accept-Ranges','bytes');return new Response(chunk,{status:206,statusText:'Partial Content',headers})}
...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'){
    event.respondWith(networkFirst(req,CORE_CACHE).catch(async()=>{
      ...
      return (await caches.match(admin?'./admin-v239.html':'./app-v239.html'))||(await caches.match(admin?'./admin.html':'./index.html'))||(await caches.match(OFFLINE_PAGE));
    }));return;
  }
  event.respondWith((async()=>{
    if(isProtectedSupabase(url)){const media=await mediaFromCache(req);if(media)return media;return fetch(req,{cache:'no-store'})}
    ...
    try{return await fetch(req)}catch(e){const media=await mediaFromCache(req);return media||(await caches.match(req))||(await caches.match(OFFLINE_PAGE))}
  })());
});

async function downloadUrl(url){const parsed=new URL(url,self.location.origin);if(!['http:','https:'].includes(parsed.protocol))throw new Error('Unsupported offline URL');if(parsed.pathname.includes('/nh7-school-media/')||parsed.pathname.includes('/nh7-school-media-access'))throw new Error('Protected videos cannot be stored offline');const cache=await caches.open(MEDIA_CACHE),key=stableMediaKey(url),existing=await stableCachedResponse(url);if(existing){const bytes=Number(existing.headers.get('content-length')||0)||(await existing.clone().blob()).size;return{bytes,already:true,identity:canonicalMediaIdentity(url)}}const res=await fetch(url,{cache:'no-store'});if(!res.ok)throw new Error('HTTP '+res.status);const bytes=Number(res.headers.get('content-length')||0);await cache.put(key,res.clone());const finalBytes=bytes||Number((await res.blob()).size||0);return{bytes:finalBytes,already:false,identity:canonicalMediaIdentity(url)}}
async function removeUrl(url){const cache=await caches.open(MEDIA_CACHE),identity=canonicalMediaIdentity(url),keys=await cache.keys();let removed=0;for(const candidate of keys){const isStable=new URL(candidate.url).pathname.includes('/'+STABLE_MEDIA_PATH+'/');if((isStable&&candidate.url===stableMediaKey(url).url)||(!isStable&&canonicalMediaIdentity(candidate.url)===identity)){if(await cache.delete(candidate))removed++}}return removed}
async function mediaStats(){const cache=await caches.open(MEDIA_CACHE),keys=await cache.keys();let bytes=0,count=0;for(const key of keys){if(!new URL(key.url).pathname.includes('/'+STABLE_MEDIA_PATH+'/'))continue;const response=await cache.match(key);if(response){count++;bytes+=Number(response.headers.get('content-length')||0)||(await response.clone().blob()).size}}const core=await caches.open(CORE_CACHE);return{mediaCount:count,mediaBytes:bytes,coreCount:(await core.keys()).length}}
self.addEventListener('message',event=>{
  ...
  event.waitUntil((async()=>{try{
    ...
    if(d.type==='REMOVE_URL'){const removed=await removeUrl(d.url);reply({ok:true,removed});return}
    if(d.type==='MEDIA_STATUS'){const r=await stableCachedResponse(d.url);reply({ok:true,cached:!!r,identity:canonicalMediaIdentity(d.url)});return}
