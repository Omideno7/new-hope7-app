...
async function idbGet(identity){const db=await openDb();return new Promise((resolve,reject)=>{const tx=db.transaction(DB_STORE,'readonly'),req=tx.objectStore(DB_STORE).get(identity);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error)})}
...
async function idbAll(){const db=await openDb();return new Promise((resolve,reject)=>{const tx=db.transaction(DB_STORE,'readonly'),req=tx.objectStore(DB_STORE).getAll();req.onsuccess=()=>resolve(req.result||[]);req.onerror=()=>reject(req.error)})}
...
self.addEventListener('message',event=>{const data=event.data||{},port=event.ports&&event.ports[0],reply=value=>port&&port.postMessage(value);event.waitUntil((async()=>{try{if(data.type==='SKIP_WAITING'){await self.skipWaiting();reply({ok:true});return}if(data.type==='CACHE_CORE'){reply({ok:true,cached:await cacheCore()});return}if(data.type==='MEDIA_STATUS'){reply({ok:true,cached:!!await offlineFullResponse(data.url),identity:canonicalMediaIdentity(data.url)});return}if(data.type==='REMOVE_URL'){await removeUrl(data.url);reply({ok:true});return}if(data.type==='CLEAR_MEDIA'){await caches.delete(MEDIA_CACHE);await caches.open(MEDIA_CACHE);try{for(const row of await idbAll())await idbDelete(row.identity)}catch(_){}reply({ok:true});return}if(data.type==='OFFLINE_STATUS'){reply(Object.assign({ok:true},await mediaStats()));return}reply({ok:false,error:'Unknown offline command'})}catch(error){reply({ok:false,error:error?.message||String(error)})})())});
